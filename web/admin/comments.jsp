...
<%@ page import="java.util.Locale"
 ...
 %>
  ...
  <%
        ...
        String sql="select t.comment_id, t.user_id, t.article_id,t.comment_text, To_Char(t.creation_date)creation_date from hr_comments t";
...
<html lang="en">
  ...
  <tbody>
    ...
    %>
    <tr>
      <td><%= rst.getString("comment_id")%></td>
      <td><%= rst.getString("article_id")%></td>
      <td><%= rst.getString("user_id")%></td>
         <td><%= rst.getString("comment_text"   )%></td>
        <td><%= rst.getString("creation_date")%></td>
  <form action="../operations/del_comment.jsp" method="post"> <td><button type="submit" value="<%= rst.getString("comment_id")%>" name="comment_id" class="btn btn-primary" style="border-radius: 8px;"  >Delete</button></td> </form>
    </tr>
